...
exports.create = (req, res) => {
    ...
    oProfile.save(err => {
        if (err) {
            ...
            res.status(200).send({ success: true, data: oProfile });
        }
    });
};

exports.update = (req, res, next) => {
    Profiles.findOne({name: req.params.name})
    .populate('creator')
    .populate('status')
    .exec((err, response) => {
        if (err) {
            ...
            if (response) {
                ...
                response.description = req.body.description || response.description;
